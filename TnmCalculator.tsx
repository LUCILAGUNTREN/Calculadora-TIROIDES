...
const TnmCalculator: React.FC = () => {
  const [inputs, setInputs] = useState<TnmInputs>({ edition: '8', age: 50, t: 'T1a', n: 'N0', m: 'M0' })
  ...
  const { tnmStage, criteria } = useMemo(() => {
    ...
    if (edition === '8') {
      const ageCutoff = 55
      criteriaText = `Usando AJCC 8ª Edición (corte: ${ageCutoff} años).`
      if (age < ageCutoff) {
        ...
        if (m === 'M1') {
          ...
          if (isT1T2 && (n === 'N0' || n === 'NX')) stage = 'Estadio I'
          else if ((isT1T2 && (n === 'N1a' || n === 'N1b')) || isT3) stage = 'Estadio II'
          else if (t === 'T4a') stage = 'Estadio III'
          else if (t === 'T4b') stage = 'Estadio IVA'
        }
      }
    } else {
      const ageCutoff = 45
      criteriaText = `Usando AJCC 7ª Edición (corte: ${ageCutoff} años).`
      if (age < ageCutoff) {
        stage = m === 'M1' ? 'Estadio II' : 'Estadio I'
      } else {
        if (m === 'M1') {
          ...
          if (isT1 && (n === 'N0' || n === 'NX')) stage = 'Estadio I'
          else if (isT2 && (n === 'N0' || n === 'NX')) stage = 'Estadio II'
          else if ((isT3 && (n === 'N0' || n === 'NX')) || ((isT1 || isT2 || isT3) && n === 'N1a')) stage = 'Estadio III'
          else if (t === 'T4a' || ((isT1 || isT2 || isT3) && n === 'N1b')) stage = 'Estadio IVA'
          else if (t === 'T4b') stage = 'Estadio IVB'
        }
      }
    }
  ...
  return (
    <>
      <Card title="Estadificación TNM" description="Calcule el estadio AJCC según T, N, M y edad.">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
          ...
          <SelectInput label="Estadio N" name="n" value={inputs.n} onChange={handleChange} options={[
            { value: 'NX', label: 'NX (No evaluable)' }, { value: 'N0', label: 'N0 (Sin metástasis regionales)' }, { value: 'N1a', label: 'N1a (Compart. central)' }, { value: 'N1b', label: 'N1b (Lateral/mediastínica)' }
          ]} />
