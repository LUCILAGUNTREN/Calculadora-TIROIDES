...
const BethesdaCalculator: React.FC = () => {
  ...
  return (
    <Card title="Sistema Bethesda para Citología Tiroidea" description="Riesgo de malignidad y manejo típico por categoría.">
      <SelectInput
        label="Seleccione la Categoría Bethesda"
        value={category}
        onChange={(e) => setCategory(e.target.value)}
        options={Object.keys(BETHESDA_DATA).map(key => ({
          value: key, label: BETHESDA_DATA[key].name
        }))}
      />
      {selectedCategory && (
