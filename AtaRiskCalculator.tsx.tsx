...
const AtaRiskCalculator: React.FC = () => {
  ...
  const { riskCategory, riskFactors, color } = useMemo(() => {
    ...
    if (hasDistantMets) { category = 'High'; factors.push('Metástasis a distancia (M1)') }
    if (incompleteResection) { category = 'High'; factors.push('Resección tumoral incompleta') }
    if (ete === 'gross_major') { category = 'High'; factors.push('ETE macroscópica a estructuras mayores (T4)') }
    if (nStage !== 'N0' && largestNode === '>3') { category = 'High'; factors.push('Cualquier ganglio > 3 cm') }

    if (category !== 'High') {
      if (aggressiveHistology) { category = 'Intermediate-High'; factors.push('Histología agresiva') }
      if (vascularInvasion === 'extensive') { category = 'High'; factors.push('Invasión vascular extensa') }
      if (ete === 'gross_strap') { category = 'Intermediate-High'; factors.push('ETE macroscópica a músculo pretiroideo (T3b)') }
      if (nStage !== 'N0' && largestNode === '1-3') { category = 'Intermediate-High'; factors.push('Ganglios 1-3 cm') }
    ...
    let display = 'border-green-400 bg-green-50'
    if (category === 'Low-Intermediate') display = 'border-yellow-400 bg-yellow-50'
    if (category === 'Intermediate-High') display = 'border-orange-400 bg-orange-50'
    if (category === 'High') display = 'border-red-400 bg-red-50'

    return { riskCategory: category, riskFactors: factors, color: display }
  }, [inputs])

  const checkboxes = [
    { name: 'isIMPC', label: 'IMPC', tooltip: 'Múltiples focos PTC ≤1 cm confinados a tiroides.' },
    ...
    { name: 'aggressiveHistology', label: 'Histología Agresiva', tooltip: 'Células altas (>30%), columnares, hobnail, sólida/trabecular, Hürthle con invasión vascular extensa.' }
  ]

  return (
    <Card title="Estratificación de Riesgo de Recurrencia ATA 2025" description="Riesgo de recurrencia tras la terapia inicial.">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
        ...
        <SelectInput label="Extensión Extratiroidea" name="ete" value={inputs.ete} onChange={handleChange} options={[
          { value: 'none', label: 'Ninguna' }, { value: 'microscopic', label: 'Microscópica' }, { value: 'gross_strap', label: 'Macro a músc. pretiroideo' }, { value: 'gross_major', label: 'Macro a estructuras mayores' }
        ]} />
        ...
        {inputs.nStage !== 'N0' && <>
          ...
          <SelectInput label="Ganglio más Grande" name="largestNode" value={inputs.largestNode} onChange={handleChange} options={[
      ...
      <ResultDisplay title="Categoría de Riesgo ATA" color={color}>
        <p className="text-2xl font-bold">
          {riskCategory === 'Low' ? 'Bajo' : riskCategory === 'Low-Intermediate' ? 'Bajo-Intermedio' : riskCategory === 'Intermediate-High' ? 'Intermedio-Alto' : 'Alto'}
        </p>
