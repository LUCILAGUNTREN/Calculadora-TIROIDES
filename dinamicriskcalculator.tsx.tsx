...
const DynamicRiskCalculator: React.FC = () => {
  ...
  const { responseCategory, criteria, color } = useMemo(() => {
    ...
    if (imaging === 'suspicious') {
      ...
      criteriaText = 'Evidencia estructural de enfermedad.'
      ...
      if (initialTx === 'total_rai') {
        if ((!isNaN(stimTgNum) && stimTgNum >= 10) || (!isNaN(tgNum) && tgNum > 1) || tgAb === 'rising') {
          ...
          criteriaText = 'Tg estimulada ≥10, Tg no estimulada >1, o AcTg en aumento.'
          ...
          criteriaText = 'Hallazgos no específicos o Tg estimulada 1-9.9 o AcTg estables/descendentes.'
          displayColor = 'border-yellow-400 bg-yellow-50'
        }
      } else if (initialTx === 'total_no_rai') {
        if ((!isNaN(tgNum) && tgNum > 5) || tgAb === 'rising') {
          category = 'Biochemically Incomplete'
          criteriaText = 'Tg no estimulada > 5 ng/mL o AcTg en aumento.'
          displayColor = 'border-orange-400 bg-orange-50'
          ...
          category = 'Indeterminate'
          criteriaText = 'Hallazgos no específicos o Tg 2.5-5 o AcTg estables/descendentes.'
          displayColor = 'border-yellow-400 bg-yellow-50'
        }
      } else {
        if (imaging === 'negative' && tgAb === 'negative') {
          category = 'Excellent'
          criteriaText = 'Lóbulo contralateral sin hallazgos de alto riesgo y sin adenopatías anormales.'
          displayColor = 'border-green-400 bg-green-50'
  ...
  return (
    <Card title="Riesgo Dinámico (Respuesta a Terapia)" description="Categorice la respuesta a la terapia inicial.">
